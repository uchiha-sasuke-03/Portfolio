...
export const SectionCharacter = ({ imagePath, name, side = 'right', className = "" }: SectionCharacterProps) => {
  return (
    <>
      {/* Desktop: Large absolute-positioned character (unchanged) */}
      <motion.div 
      ...
      {/* Mobile: Compact inline character card */}
      <motion.div
      ...
      >
        <div className="relative">
          <div className="absolute inset-0 bg-pop-yellow border-3 border-black translate-x-2 translate-y-2 -z-10" />
          <div className="border-3 border-black bg-white p-1.5 overflow-hidden w-32 h-40 sm:w-40 sm:h-48 shadow-[4px_4px_0px_#000]">
            <img 
              src={imagePath} 
              alt={name} 
          ...
          <div className="absolute -bottom-3 left-1/2 -translate-x-1/2 bg-black text-white font-heading text-xs sm:text-sm px-3 py-0.5 uppercase tracking-wider whitespace-nowrap">
            {name}
          </div>
        </div>
      </motion.div>
